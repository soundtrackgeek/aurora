...
Opening an album displays tracks and saved rating/Love edits from the local catalog first. Pending music-file genres and deletions reconcile in the background for that album, so a slow mounted share does not hold the track list behind “Opening the deep catalog”. Until reconciliation finishes, pending file changes may still show their imported values; a failed refresh leaves the local tracks visible. Album timing stages are recorded in `aurora-timing.jsonl`.

Library searches (including saved startup searches) show local catalog results and saved rating-completeness changes first. Pending file genres and deletions reconcile in the background; the loaded results, count, and pagination cursor then update together. Until that completes, filters use imported file metadata. A disconnected share leaves local results usable, and a newer search or pagination request supersedes older background results.
