...
Configure **Settings → Connections → Jev & OpenRouter** with an OpenRouter API key, then use the connection test. The key stays in the operating system credential vault; debug builds can also read `OPENROUTER_API_KEY` from `.env.local` or `.env`. These files are ignored by Git; never prefix the key with `VITE_`. Release builds use the vault. Browser preview uses labeled sample albums and cannot store or test real keys.

Tonight’s Album reads at most 180 candidates from existing library/rating queries, checks up to 36 eligible albums locally, and submits at most 12 albums in one explicit request to OpenRouter’s [Decisions API](https://openrouter.ai/docs/guides/community/jev), pinned to `typesafe/jev-1.13`. [TypeSafe Score](https://docs.typesafe.ai/primitives/score) judges intention and descriptive fit. Only bounded artist/title/genre metadata, preference/progress labels, and the requested description leave the app; paths, catalog IDs, exact ratings, and listening history stay local. There are no automatic paid retries or background refreshes. Missing keys, service errors, invalid responses, or insufficient confident evidence fall back to local ranking; turning off **Ask Jev to help** always ranks locally.

Aurora computes full running time, live rating completeness, available-history recency, and file availability. Unavailable, banned, partially deleted, over-budget, and unknown-duration albums are excluded; playback rechecks the files and time budget. Finish something means a partly rated album with ratings still to complete. The bounded shortlist may produce fewer than three matches, and absence from available Aurora history is not a claim that an album has never been heard. Feedback lives in Aurora’s state database; suggestions never mutate the catalog or tags. Jev confidence is a ranking signal, not proof of musical fit.

For an explicit live smoke test against the local catalog and development key, run `cargo test --manifest-path src-tauri/Cargo.toml live_tonight_smoke -- --ignored --nocapture --test-threads=1`; it uses temporary Aurora feedback/history databases and does not start audio. Normal tests never call OpenRouter. Run `cargo clean` in `src-tauri` after Rust checks.
...
Aurora 0.26.0 adds a dedicated artist page with a photographic header, biography, MusicBrainz origin and lifespan facts, local albums and tracks, a release timeline, your last twelve months of listening, Last.fm's worldwide top ten tracks, and similar artists. Album artist links and artist rows open the page directly. **Back to library** restores the previous results and selection. Play and Shuffle use the loaded local tracks; the Tracks tab loads additional pages. Popular-track titles look for an exact local title before playing and explain when it is unavailable.

Add **fanart.tv** credentials in **Settings → Metadata** using the project API key and optional personal/VIP key. Credentials are stored in the operating system vault, never synchronized or exposed to the frontend; debug builds also accept `FANART_TV` and optional `FANART_TV_PERSONAL` from `.env.local`. Artwork uses the resolved, unambiguous MusicBrainz identity. Last.fm credentials in the same panel enable biographies, similar artists, and global popularity counts; these are distinct from your registered Aurora plays. Successful provider responses are cached locally for 24 hours in `aurora-artist-cache.sqlite3`, with saved data retained when a refresh fails. **Refresh** retries providers; **Data availability** explains missing sources.

The artist page loads through React Suspense and uses View Transitions for navigation, sections, and independently arriving data. Local catalog and MusicBrainz context stay usable while online providers load. Motion respects reduced-motion preferences. The browser preview intentionally shows provider-unavailable states unless a test fixture is supplied; production credentials only run in the desktop backend.

Inbox lets you choose one **Destination for all albums** at the top of its move dialog. Select General, Scores, or Synthwave once to apply it to the entire selection; individual album destinations can still be adjusted below. The shared selector stays visible while you scroll, shows mixed destinations when individual choices differ, and requires a new preview after any destination change.
...
Song edits accept whole stars (1–5), unrated, and Love on/off. The player can clear its current rating. Album averages may be fractional. Global rating and Love shortcuts also use this verified remote edit path while Aurora plays a track. General tag editing, Ban, and remote undo remain unavailable. The PC remains authoritative; unrelated PC edits and imports still require the existing catalog refresh. Reopen or refresh Music Library views to see local catalog changes.
...
Opening an album displays the local catalog, saved rating/Love edits, and previously verified file corrections immediately. Pending music files reconcile in the background for that album, so a slow mounted share does not hold the track list behind “Opening the deep catalog”. Album timing stages are recorded in `aurora-timing.jsonl`.

Library searches (including saved startup searches) immediately include saved rating-completeness changes and verified deletions/genres. Corrections persist in the device-local `aurora-file-observations.sqlite3` cache beside Aurora state, independently of catalog and state snapshot replacement. Only matching track paths and import revisions reuse observations; a new import invalidates older evidence. Background checks update the cache when files return or genres change. An unreadable or unavailable album folder never counts as proof of deletion and leaves prior observations intact. Unseen file changes still need one successful background check; results, counts, and cursors then update together, and a newer request supersedes older background results.
...
Network Mode blocks direct music-file writes: supported rating/Love changes, including global shortcuts, are delegated to Tonehavn; general tag edits, intake, conversion, deletion, local companion mutations and tag recovery are blocked. Listening history and player state still save locally and synchronize. macOS Now Playing handles physical media keys when Aurora has a current track, with title, artist, album, duration, and playback progress. The companion defaults to `/Applications/Music Library.app/Contents/MacOS/music-library`; `AURORA_MUSIC_LIBRARY_EXE` can override its path.
...
- Folder synchronization and Add Music intake share one bridge coordinator, so preview or apply waits behind active background work instead of racing Music Library's Windows workflow lock. Folder synchronization is token-protected: one invalid old folder cannot poison a new edit, an older receipt cannot erase a newer edit queued for the same folder, and neither a delayed edit response nor external-tag reconciliation can project over newer tag state. Pending overlays are reconciled per live track, so a targeted album import does not hide edits still awaiting synchronization in another album. The independent retry loop selects the oldest ready folder every five seconds even while Aurora is unfocused. Busy, locked, unavailable, timed-out, and companion-upgrade failures stay durably scheduled until they succeed; only permanent folder validation failures pause after three attempts, and a later MP3 edit reopens them. Startup backfills any live tag-overlay folder missing from the durable queue, and a whole-folder validation failure still gets a guarded exact-file attempt for each verified overlay MP3. The Ratings completion shelf deliberately waits for its explicit Refresh action.
...
- Field-aware search supports `artist:` (Display Artist), `aartist:` (Album Artist display), `album:`, `genre:`, `year:` (Year), `ryear:` (Release Year), `publisher:`, `country:` (Album Artist origin), `title:`, album-wide `cr:` completeness, and album-wide `love:` count filters. Every field also accepts `=` as an equivalent separator. Country accepts imported names or two-letter codes. Numeric fields accept inclusive closed or open ranges such as `year:1985..1987`, `cr:50..80`, and `love:1..3`; `cr:80`, `love:1`, and `love:0` retain their established shorthand meanings. Commas or uppercase `AND` combine groups; uppercase `OR` adds alternatives and inherits the preceding field; `NOT` or a leading `-` excludes a group. A complete quoted value is exact, while unquoted text remains prefix-based. `genre:scores` expands to the Music Library film, TV, animation, anime, and game-score genres.
...
**Remove Album** is available in expanded album details beside **Move to Inbox**, and at the bottom of the Album inspector. The inspector works in every library Albums view, including Universe, Songs, Artists, and Tags. Removal moves the complete album folder to `D:\MUSIC_NOT_ALBUMS\`, outside `D:\MUSIC`, verifies the copy, removes the album through Music Library’s transactional importer, and cleans the source only after commit. The current Albums view immediately removes the row and refreshes. A confirmation shows the destination; collisions and changed sources are rejected, and cleanup warnings retain recovery information. The new destination requires Aurora 0.26.7 and Music Library 0.150.7 or newer; Aurora rejects previews from helpers still using the old location.
