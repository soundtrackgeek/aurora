...
export type CompletionKind = "partiallyRated" | "unrated";
...
const PREVIEW_ALBUMS: readonly RatingAlbum[] = [
  ...
  { id: "preview-hurry-up", title: "Hurry Up, We're Dreaming", artist: "M83", originalYear: 2011, releaseYear: 2011, genre: "Electronic", totalTracks: 22, ratedTracks: 15, lovedTracks: 7, durationSeconds: 4402, remainingTracks: 7, effectiveRating: null, provisionalRating: 4.4, albumScore: null },
...
const PREVIEW_COMPLETION: CompletionCounts = {
  partiallyRated: 6_401,
  unrated: 59_578,
};

function previewPage(kind: CompletionKind, remainingTracks: number | null = null): RatingAlbumPage {
  let albums = kind === "partiallyRated"
    ? [...PREVIEW_ALBUMS]
    : PREVIEW_ALBUMS.map((album) => ({ ...album, id: `${album.id}:unrated`, ratedTracks: 0, lovedTracks: 0, remainingTracks: album.totalTracks, provisionalRating: null }));
  albums = albums.map(syncPreviewAlbum);
  if (kind === "partiallyRated" && remainingTracks !== null) {
    albums = albums.filter((album) => album.remainingTracks === remainingTracks);
  }
  if (kind === "unrated") {
    albums.sort((left, right) => (right.originalYear ?? -1) - (left.originalYear ?? -1) || left.title.localeCompare(right.title));
  }
  return { kind, total: remainingTracks === null ? PREVIEW_COMPLETION[kind] : albums.length, albums };
}
...
function previewOverview(): RatingsOverview {
  return {
    ...
    initialPage: previewPage("partiallyRated"),
  };
}
...
export async function loadRatingAlbumPage(kind: CompletionKind, remainingTracks: number | null = null): Promise<RatingAlbumPage> {
  if (!isTauriRuntime()) return previewPage(kind, remainingTracks);
  return invoke<RatingAlbumPage>("rating_album_page", { kind, remainingTracks });
}
