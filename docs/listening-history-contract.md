...
Aurora scans `%APPDATA%\com.local.musiclibrary\tonehavn-history` for the same validated snapshot contract and atomically mirrors valid `tonehavn-local` and `tonehavn-ios` snapshots into the bounded `_musicbackup\tonehavn-history` OneDrive child as backups. The live local journals remain canonical and Aurora never writes them; when present, they take precedence over same-named OneDrive backups so sessions are not duplicated. Their device names appear in the existing device filter and their registered plays participate in the same timeline, report, top-track, genre, and track-insight aggregation as Aurora-native peers.
