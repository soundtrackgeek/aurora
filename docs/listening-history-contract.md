...
Aurora also scans the bounded `_musicbackup\tonehavn-history` child directory for the same validated snapshot contract. Tonehavn owns and publishes `tonehavn-local` and `tonehavn-ios`; Aurora never writes those journals. Their device names appear in the existing device filter and their registered plays participate in the same timeline, report, top-track, genre, and track-insight aggregation as Aurora-native peers.
