...
Aurora 0.17.0 edits MusicBee-compatible metadata in MP3 files while keeping `%APPDATA%\com.local.musiclibrary\music-library.sqlite3` strictly read-only.
...
Aurora never updates Music Library's database directly. After verified MP3 writes it asks Music Library `0.144.0` or newer to synchronize only the affected existing album folders:
...
Music Library guarded existing-folder sync
        ↓
completed import revision → Aurora refreshes catalog-backed views
```

Music Library accepts at most 32 folders and 50,000 cataloged MP3s per bridge request, requires every folder to remain inside a configured library root, rejects linked/reparse paths, preserves album identity, and refuses any preview that would add or remove tracks or albums. Aurora durably queues each affected folder in the same state transaction that verifies the MP3 write or undo. If the companion is missing, outdated, or rejects the sync, the already-verified MP3 save remains successful and Aurora reports catalog synchronization as pending; startup, focus, and later saves retry the queue until Music Library acknowledges it.
